    ...
    async search(query: string, max: number = 20) {
        ...
        for (const account of accounts) {
            ...
            try {
                ...
                const searchProc = Bun.spawn(["bash", "-c", searchCmd], { stdout: "pipe", stderr: "inherit" });
                ...
                for (const thread of threads) {
                    ...
                    const existing = await db.select({ id: jobs.id }).from(jobs).where(eq(jobs.threadId, thread.id)).get();
                    if (existing) {
                    ...
                    try {
                        ...
                        const getProc = Bun.spawn(["gog", "gmail", "thread", "get", thread.id, "--account", account, "--json"], { stdout: "pipe", stderr: "inherit" });
                        ...
                        try {
                            ...
                            console.error(`Failed to parse thread ${thread.id}: ${getText.substring(0, 100)}...`);
                            continue;
                        }

                        if (!threadDetails.messages && (!threadDetails.thread || !threadDetails.thread.messages)) {
                            console.warn(`⚠️ Thread ${thread.id} has no messages. Response keys: ${Object.keys(threadDetails)}`);
                            continue;
                        }

                        const messages = (threadDetails.thread && threadDetails.thread.messages) || threadDetails.messages || [];
                        console.log(`[Gmail] Thread ${thread.id}: Found ${messages.length} msgs.`);
                        ...
                        for (const message of messages) {
                            ...
                            if (message.payload) {
                                body = findHtmlBody(message.payload) || findTextBody(message.payload) || "";
                            ...
                            const emailData = {
                                ...
                                subject: message.payload?.headers?.find((h:any) => h.name === 'Subject')?.value || thread.subject || "",
                                from: message.payload?.headers?.find((h:any) => h.name === 'From')?.value || thread.from || "",
