import { CAC } from "cac";
import { EvaModule } from "../../types/module";
import { db } from "../../db";
...
export class JobModule implements EvaModule {
    name = "jobs";
    ...
    registerCommands(cli: CAC) {
        cli.command("jobs:status", "Show job hunting dashboard")
           .action(() => this.showStatus());

        cli.command("jobs:enrich", "Enrich jobs with Firecrawl")
           .action(() => this.enrich());

        cli.command("jobs:analyze", "Analyze jobs with LLM")
           .action(() => this.analyze());
        
        cli.command("jobs:list", "List all jobs")
           .action(() => this.listJobs());

        cli.command("jobs:resume <path>", "Import resume (MD or PDF)")
           .action((path) => this.importResume(path));
    }

    // --- Morning Briefing ---
    async getDailyBriefing(): Promise<string> {
        const newJobs = await db.select().from(jobs).where(eq(jobs.status, 'New')).orderBy(desc(jobs.receivedAt)).all();
        ...
        for (const job of newJobs.slice(0, 10)) {
            ...
            if (job.analysis) {
                ...
                const strategy = lines.find(l => l.includes("Strategy"))?.replace(/- \*\*Strategy\*\*:/, "").trim();
...
💡 *Run 'eva jobs:enrich' to grab JDs.*
💡 *Run 'eva jobs:analyze' to get AI fit reports.*
`);
    }
    ...
    private async analyze() {
        ...
        const targets = await db.select().from(jobs).where(and(isNotNull(jobs.description), isNull(jobs.analysis))).all();
        ...
        else console.log("⚠️ No Resume configured. Analysis will be generic. (Use 'jobs:resume <path>' to set)");
    ...
    private async listJobs() {
        const rows = await db.select().from(jobs).orderBy(desc(jobs.receivedAt)).limit(20);
        ...
        for (const job of rows) {
            ...
            console.log(`### ${statusIcon} ${job.role || 'Unknown'} @ ${job.company || 'Unknown'} ${enrichedIcon}${analyzedIcon}${scoreStr}`);
        ...
        try {
            ...
            if (filePath.toLowerCase().endsWith(".pdf")) {
                ...
                if (content.trim().length === 0) {
                     ...
                     console.log(`✅ Extracted ${content.length} characters from PDF (via pdftotext).`);
                }
            } else {
